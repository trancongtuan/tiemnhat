...
const MasterProductDetail = ({ product, productDetail, currency, uniqueTags, detailClass, title, des, variantChangeByColor }: any) => {
  let RatingStars = [];
  let rating = 5;
  ...
  return (
    <div className={`product-detail ${productDetail} ${detailClass}`}>
      <div>
        {title !== 'Product style 4' ? <div className="rating">{RatingStars}</div> : ''}
        <h6>{product.title}</h6>
        ...
        {product.variants.map((vari: any) => {
          var findItem = uniqueTags.find((x: any) => x.color === vari.color);
          ...
          <>
            {title !== 'Product style 4' && uniqueTags[0].color ? (
              <ul className="color-variant">
                {uniqueTags.map((vari: any, i: number) => {
                  return (
                    <li
